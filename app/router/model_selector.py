...
class ModelSelector:
    ...
    def _score_capability(self, model: dict, features: dict, complexity: float) -> float:
        """Score how well a model can handle the required capabilities"""
    ...
    def rank_models(self, features: dict, complexity: float) -> List[Dict[str, Any]]:
        """Rank models by cost-effectiveness for the given query"""
        models = settings.models
        config = settings.routing_config
        weights = config.get("weights", {})

        # Adjust weights to favor cost efficiency
        w_cap = weights.get("capability_weight", 0.4)  # Lower capability weight
        w_cost = weights.get("cost_weight", 0.5)  # Higher cost weight (token-efficient!)
        w_lat = weights.get("latency_weight", 0.05)
        w_succ = weights.get("success_weight", 0.05)

        est_tokens = features.get("est_tokens", 0)
        # Better output token estimation: complex queries need more output
        if complexity > 0.7:
        ...
        est_total_tokens = est_tokens * (1 + output_multiplier)
        ...
        for model in models:
            ...
            output_cost = model.get("output_cost_per_1k", 0) * (est_tokens * output_multiplier / 1000.0)
            est_cost = input_cost + output_cost

            # Better cost normalization - use 0.05 as reasonable max
            norm_cost = min(est_cost / 0.05, 1.0) if est_cost > 0 else 0.0
