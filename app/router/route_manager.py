...
class RouteManager:
    ...
    def route_query(self, query: str) -> dict:
        ...
        # 4. Rank Models (cost-optimized selection)
        ...
        threshold = settings.routing_config.get("thresholds", {}).get("confidence_threshold", 0.85)
        max_retries = settings.routing_config.get("thresholds", {}).get("max_retries", 1)
        ...
        # 5. Execute with Cost-Aware Confidence-Based Selection
        best_result = None
        best_confidence = -1.0
        selected_model_name = ""
        ...
        for i, candidate in enumerate(ranked_models):
            ...
            if result["success"]:
                ...
                # Early exit if confidence is sufficient (avoid wasting tokens on retries)
        ...
        if best_result:
            ...
            explanation["reason"] = f"Cost-optimized selection with confidence {best_confidence:.2f}"
            best_result["explanation"] = explanation

            # Save to Cache only if confidence is reasonable
            if best_confidence >= (threshold - 0.15):
